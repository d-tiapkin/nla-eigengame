...
def eg_grads(vi: jnp.ndarray, weights: jnp.ndarray, eigs: jnp.ndarray, data: jnp.ndarray) -> jnp.ndarray:
    """
    Args:
...
def _grads_and_update(vi, weights, eigs, input, opt_state, axis_index_groups):
    ...
    Args:
    vi: shape (d,), eigenvector to be updated
    weights: shape (k_per_device, k,), mask for penalty coefficients, eigs: shape (k, d), i.e., vectors on rows
...
def step(self, *, Key = None):
    ...
    self._V, self._weights_jnp, self._local_V, inputs, self._opt_state, global_step)